...
本專案的執行環境實測過：**WSL2 VM 的牆鐘約每 30 秒被校正一次、每次往回跳約 1.5 秒**
（Windows 原生 30 秒內 0 次回跳）。細節見
[負載特性報告](./performance-report.md) 與 Week 8 的工作記錄。

1.5 秒相對於 60 秒租約是 2.5%，實務上不足以造成雙重持有。但這個風險是**結構性的**：

- 若某個副本的時鐘**走快**，它會提早認為租約已過期而搶鎖 —— 而真正的持有者還在執行。
- 租約越短，時鐘誤差的相對影響越大。若把租約縮到 5 秒，1.5 秒的跳動就是 30%。
