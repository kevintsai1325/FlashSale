...
**`reservationMaxMs` 讀到 0，不代表 Redis 變快了，這個指標的回報率是隨到達率逐步崩潰的，
不是一刀切。** 把三個 sweep 的 `downstream-saturation-r{1,3,5}-*.jsonl` 逐樣本攤開看：

- **150 rps：每一個樣本都讀到非零值**——`replicas=1` 49 個樣本全部是同一個值 32.09ms；
  `replicas=3` 48 個樣本落在 3 個不同值（3.52／37.57／41.64ms）；`replicas=5` 49 個樣本落在
  5 個不同值（1.83～47.59ms）。前面提過的 32.09／41.64／47.59ms，其實是**這個速率下觀察到
  的最大值**，不是一個穩定不變的讀數——`replicas=5` 在 150 rps 就已經在 1.83ms 到 47.59ms
  之間跳動。
- **300 rps：已經有相當比例的樣本讀到 0**——`replicas=1` 49 個樣本裡 13 個是 0（27%）；
  `replicas=3` 48 個裡 17 個是 0（35%）；`replicas=5` 49 個裡 19 個是 0（39%），其餘樣本仍
  落在跟 150 rps 相同的那組非零值裡（`replicas=5` 甚至多觀察到一個新值 1.39ms，變成 6 個
  不同值）。
- **600 與 900 rps：三個 sweep、所有樣本（47–49 個不等）全部讀到 0.000000**，沒有例外。

這個「150 rps 全部回報 → 300 rps 大約三分之一開始變 0 → 600/900 rps 全滅」的漸進式崩潰，
比一刀切的斷點更難用「Redis 剛好在某個負載點瞬間變快」來解釋，反而更像是**這個指標本身
隨負載升高逐漸失去回報能力**：`purchase.reservation.latency` 是 Micrometer 的 `Timer`，
其 `MAX` 統計量會隨捲動視窗衰減；`sample-downstream.ps1` 的 `Get-MetricValue` 在指定的
統計量不存在時只會靜默回傳 `null`，取樣器再把 `null` 收斂成 0——負載越高，視窗內平均間隔
越可能超過取樣週期，統計量沒回報、被收斂成 0 的樣本比例也就越高，直到 600 rps 以上全部
如此。**結論：Redis 在有負載的速率下實際上沒有被這個指標可靠測到，不能用
...
的 `reservationMaxMs`（`replicas=1`／`3` 在 600、900 rps 的每一個樣本都讀到 0，理由見上一節：
這個指標的回報率隨到達率升高逐步崩潰，600 rps 以上已經全滅，不是 Redis 真的是 0ms）。
48.9ms 本身不慢，但因為其他組別在這個速率下已經測不到，這裡只能說「五副本、每副本負載
最輕的情況下 Redis 預扣是快的」，不能推廣成「Redis 在任何負載下都不是瓶頸」。`pgBackends`
峰值 **152**——非常接近
...
**本節記錄的是實驗當時的設定，不是現在 repo 裡的設定。** `k8s/autoscaling/hpa.yaml` 現在是
`minReplicas=2`、`maxReplicas=3`：這個專案要練的是多副本共用狀態的正確性（分散式鎖、排程
Lease、庫存扣減），那件事在 2~3 個副本就完整成立，不需要開到 8 個。把上限收到 3 也讓下面那場
重啟風暴在結構上不可能再發生——一次擴容只多啟動 1 個 JVM，而不是 5 個同時冷啟動。
下面的數字與結論仍然有效，但它們描述的是「3→8 這種幅度的擴容」，不是現在的設定會做的事。
...
**已處理的一項**：backend 的 `livenessProbe` 現在明寫 `timeoutSeconds: 5`
（`k8s/base/application.yaml`），不再繼承 Kubernetes 的 1 秒預設值——上一節機制分析指認的
第一個原因就是它。**本節的數字是在舊設定下量的，沒有在新設定下重測**，所以不能宣稱重啟風暴
已經解決；能宣稱的只有「被 CFS 節流的 JVM 現在有 5 秒而不是 1 秒可以回應探測」。
另外兩項（更保守的 `scaleUp` 階梯、多節點環境重測）仍未做。
...
自己也需要被納入容量規劃**，尤其是全量取樣加高流量的組合。

**後續處理**：`k8s/base/support.yaml` 現在把 Zipkin 的 `MEM_MAX_SPANS` 從預設的 500000 壓到
50000，直接限制記憶體內儲存的保存量，而不是調低取樣率（取樣率調低會讓平常的追蹤示範失去
代表性）。**這個設定尚未在壓測負載下重新驗證**——它是依 OOM 的機制推出來的修正，不是量到的
結果；下一次跑飽和壓測時要一併確認 Zipkin 的重啟次數。真正的解法仍然是換成有儲存後端的部署。
