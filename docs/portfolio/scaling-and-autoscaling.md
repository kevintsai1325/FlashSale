...
是完全符合預期的結果，但它回答的問題是「固定併發下三副本比較慢嗎」，不是「水平擴展有沒有
用」。這是一個**封閉模型（closed model）造成的量測假象**，不是水平擴展本身無效的證據——
...
`replicas=5`、`targetRate=900`（重跑版本，見第 7 節的重跑說明）：同樣 47 個樣本、
...
`kubectl get events` 顯示 16:41:18 之後，原本已經健康跑了 4 分鐘以上的 3 個舊 Pod，以及全部
5 個新 Pod，**全數**因 liveness probe timeout（`context deadline exceeded`）被 kubelet 殺掉
重啟。8 個 Pod 在 16:42:08–16:42:19 之間**同時 NotReady**——這是一次真正的服務中斷，不是
單純變慢。
...
測試叢集上是自傷性的：同時冷啟動多個 JVM 造成的節點級 CPU 搶佔，觸發了波及全部 8 個 Pod
（含未參與擴容的舊 Pod）的重啟風暴與短暫服務中斷。這是一個負面但誠實的結果。
