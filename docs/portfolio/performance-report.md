...
**當時的結論:** 這個問題看起來同時受 Tomcat backlog 大小**和**同機資源競爭影響,把兩者都排除
才拿到乾淨結果,單獨調大 `accept-count` 沒有可靠地解決它。也就是說,`accept-count` 從 100 調到
300 這個動作本身有沒有必要、還是只是同機干擾恰好那幾次沒踩到,**沒有做隔離實驗分開驗證**。

#### 2026-09-13 補記:根因找到了,而且不是 Tomcat

上面那個「沒有做隔離實驗」的缺口後來補上了。做法是把施壓端移到容器網路內,讓同一個 backend
容器同時有兩條被壓的路徑可以對照:

| 施壓來源 | 路徑 | 300 條同時新連線 | 被拒 |
|---|---|---|---|
| k6 在容器網路內 | → `backend:8080` | 是 | **0 / 300** |
| k6 在 Windows | → `127.0.0.1:18080` | 是 | **75 / 300** |

兩次執行相隔數秒、同一個容器、同一支腳本。另外在 k3s 上用單一 backend Pod、刻意把
`accept-count` 設回預設的 100，從叢集內打 300 條同時新連線也是 **0 失敗**。

**更正**：本節原本寫「從 Windows 經 NodePort 打 2000 條同時連線同樣 0 失敗」。
2026-09-13 重新量測**推翻了這個說法** —— 從 Windows 經 k3s NodePort 打 300 條同時新連線，
被拒 90 條，與 Compose 的 host port 是同一個症狀。原本那次量測應該是在連線重用或速率斜坡的
情況下取得的，不是真正的「同時」。

### Windows 到 WSL2 的埠轉發層：實際容量

2026-09-13 在 k3s NodePort 上重新量測（每條都是全新連線，`noConnectionReuse`）：
...
| 瞬間同時連線 | 300 | 210 | 90 |
| 瞬間同時連線 | 400 | 207 | 193 |
| 持續新連線速率 | 400 / 秒 | 5,114 | **0** |
| 持續新連線速率 | 800 / 秒 | 10,108 | **0** |
| 持續新連線速率 | 1,200 / 秒 | 15,100 | **0** |
| 持續新連線速率 | 2,000 / 秒 | 23,070 | 1,975 |
| 持續新連線速率 | 3,000 / 秒 | 17,218 | 20,288 |

同一時間從 VM 內部對同一個 NodePort 打 300 條同時連線：**300 / 300 全通**。

**這張表說明兩件事：**

1. **被卡住的是「同時建立中的連線數」，不是連線速率。** 不論打 250、300 還是 400 條，
   被接受的都是約 210 條 —— 那是一個固定的上限，超出的部分在 TCP 握手階段就收到 RST。
   但把同樣多的連線攤在時間上，每秒 1,200 條新連線可以 0 失敗。
2. **「300 個請求只收到 2 百初」這個現象有了精確的數字：210。**

在 Windows 上持有 `127.0.0.1:<NodePort>` 這個 listener 的是一個 Rancher Desktop 的使用者空間
中繼行程，不是 WSL2 核心。也試過 WSL2 的 `networkingMode=mirrored`（Windows 11 build 26200
支援），**沒有改善**（同樣約 210），因為 Rancher Desktop 仍以自己的行程轉發 Kubernetes 服務埠。

**根因:連線是被 Windows 的 host port 發布層拒絕的,不是被 Tomcat 拒絕的。** 那一層的 listen
backlog 撐不住 300 條瞬間到達的新連線,在 TCP 握手階段就回 RST,Tomcat 從頭到尾沒看到這些連線。

這解釋了上面三輪嘗試裡兩個一直說不通的地方:

- **為什麼調到 512 反而更差**——因為調的是一個不在瓶頸路徑上的參數,結果自然是隨機的。
- **為什麼停掉同機閒置的專案有效**——同機資源競爭正是**透過這一層**作用的。兩個看似獨立的
  因素其實是同一件事。

也就是說,`server.tomcat.accept-count` 與 `threads.max` 的調整對這個症狀**沒有作用**。
(同一個 commit 裡把 Hikari pool 從 10 調到 30 是另一回事,那個有獨立證據支持,見下方第 4 點。)

**修正方式**:`load-tests/benchmark/collect.ps1` 已改為以容器形式執行 k6 並接進 compose 網路,
施壓流量不再經過 host port。修正後同樣的 300 VU 情境:`accepted` 300、`failed` 0、
`acceptedLatencyMs.min` 從 0ms 變成 340.9ms。

**這份報告上方的所有數字仍然有效**——它們是在「乾淨的 5/5」那次收集到的,沒有混入失敗樣本。
但量測邊界要改寫成:當時是靠排除同機干擾**碰巧**避開了這個失真,而不是靠 Tomcat 設定。
下一次重新收集會使用容器內施壓,屆時數字會比這份報告更高一些,原因見下一段的同一個機制。
