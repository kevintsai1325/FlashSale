...
## P4-3 完成紀錄（2026-09-15）

### 驗收結果

| # | 條件 | 結果 |
|---|---|---|
| 1 | Kafka 進叢集 | KRaft 單節點 StatefulSet，兩個 topic 各 3 分區，15 個 Pod 全部 Running、0 重啟 |
| 2 | 領域事件經同一張 outbox 送上 Kafka | `OrderCreated`、`OrderStatusChanged`、`PurchaseRequestCreated`、`PurchaseRequestResolved` 實測皆到達 |
| 3 | 同一 `flashSaleId` 落在同一分區且有序 | `OrderEventsPartitioningIT` 守著；實測 `902`／`903` 的事件全部落在 partition 2 |
| 4 | 既有的 RabbitMQ 路徑不受影響 | 完整搶購流程照常，DLQ 全空 |
| 5 | `verify.ps1` 與無漂移 | 兩者皆通過 |

### 部署時抓到的缺陷：發佈器幾乎每筆事件都送兩次

Kafka 上一眼就看得出來：同一個 `orderId` 連續出現兩次，六筆訂單變成十二筆訊息。

原因是發佈器的交易結構。它原本「先撈一批並提交，再逐筆用 `REQUIRES_NEW` 發佈」，
這樣寫是為了避免巢狀交易自己鎖自己 —— 但副作用是 `SELECT ... FOR UPDATE SKIP LOCKED`
幾乎沒有作用：列鎖在發佈之前就釋放了，其他副本的下一輪輪詢照樣撈得到同一列。

**這個缺陷在拆分前一直存在，只是沒有人看得見。** RabbitMQ 的消費端本來就會去重，
重複投遞被安靜地吸收掉。直到事件被送上 Kafka、被一個會累加的下游讀到，它才顯形 ——
如果沒有在這一步發現，P6 的 GMV 會直接翻倍，而且錯得很難查。

改成整批在同一個交易裡撈、發、標記。鎖持有到交易結束，其他副本整批跳過。
代價是交易期間包含一次網路往返，所以 Kafka producer 的逾時壓到 10 秒 ——
否則 broker 掛掉時交易會連同列鎖卡住兩分鐘（`delivery.timeout.ms` 的預設值）。
順帶把「送出去要等 ack」補上：不等 ack 就標記已發佈，會從「至少一次」掉成「至多一次」。

改完之後程式反而更短：少了一個 `REQUIRES_NEW`、少了一次 refetch、少了一個方法。

### 與計畫不同的地方

- **事件多帶了「算得出來」的欄位**（`productName`、`totalAmount`）。事件是給下游用的，
  下游不該為了顯示商品名稱而回頭查 platform 的資料庫 —— 那正是 P5 要消除的跨服務查詢。
  這叫 event-carried state transfer，是拆庫之後事件設計的預設做法。
- **搶購事件連被擋下來的也發**（SOLD_OUT、REJECTED）。下游要算的是「多少人來搶、
  多少人買到」，被擋下的那些正是分母；只發成功的事件，轉換率永遠是 100%。
- **`OrderStatusChanged` 一個事件涵蓋付款／取消／逾時三種轉移**，而不是三個事件型別。
  下游關心的是「轉到哪個狀態」，三個型別只會讓每個消費者都寫一次三選一的分派。
...
| # | 條件 | 結果 |
|---|---|---|
| 1 | Flink 在叢集內運行 | JobManager + TaskManager（Application mode），22 個 Pod 全部 Running |
| 2 | 秒級 GMV、每秒訂單數、熱門 Top N | 三個數字都在 `flashsale.realtime-metrics` 上，實測每秒一筆 |
| 3 | **Flink 的 GMV 與 `order_db` 的 `SUM` 一致** | `[since, until)` 區間內 amount=495.00 count=33，兩側完全相同 |
| 4 | **重啟 TaskManager 後聚合仍正確** | 40 秒的流量進行到第 20 秒時 `--force --grace-period=0` 砍掉 TaskManager；涵蓋那一刻的區間 amount=820.00 count=55，兩側仍完全相同 |
| 5 | 大屏可用 | SSE 端點回 `text/event-stream`，連上立刻補最新快照；沒有 token 回 401 |
| 6 | `verify.ps1` 與無漂移 | 兩者皆通過 |

### 三個作業做成兩個視窗

規格寫「三個作業」。實作是兩個視窗運算元產出三個數字：GMV 與每秒訂單數在同一個
一秒視窗裡算完一起送，Top N 用十秒視窗。

理由：那兩個數字必須同時抵達大屏 —— 拆成兩個作業會讓大屏收到兩則時間戳相同、
但可能不同步抵達的訊息，畫面上就會出現「這一秒賣了 3 筆、金額 0 元」的瞬間。
Top N 則刻意用比較長的視窗：一秒的排行榜在低流量時只有零星幾筆，會抖動到無法閱讀。

### 過程中修掉的兩個部署層問題

1. **TaskManager 的 readiness 一直失敗、Pod 反覆重啟**，但日誌顯示它已經向 ResourceManager
   註冊成功。原因是 Flink 的 `taskmanager.rpc.port` 預設是 0（隨機），而隨機埠沒辦法做
   tcpSocket 探測。固定成 6122 就好。
2. **大屏一格都不動。** 把 24 筆購買一次打完，它們全部落在 watermark 還沒越過的區間 ——
   看起來像 Flink 壞了，其實是「串流沒有結束，只是暫停了」。示範腳本因此改成一秒一位買家，
   這也比較像真實的秒殺流量。

第二點是事件時間視窗最容易被誤解的地方，所以它同時寫進了程式註解、示範腳本的註解，
以及驗收腳本的說明 —— **驗收腳本比較的是「已經關閉的視窗」那一段時間，不是「到現在為止」**。
拿「到現在為止」去比，量到的是 watermark 的必然延遲，不是正確性。

### 刻意留下的缺口

- **沒有注入合成的遲到事件。** 要注入就得往 `order-events` 寫一筆假訂單，
  而 analytics 的投影會照單全收 —— 為了測一條路徑而在讀取模型裡留下一筆不存在的訂單，
  代價大於收穫。遲到路徑的證據來自兩處：`allowedLateness(10s)` 與「以 windowEnd 覆蓋」
  的下游（前端有測試守著），以及 TaskManager 重啟後的重放（那本身就是一次大規模的重算）。
- **JobManager 掛掉時 job 不會自動重新提交**（見 architecture.md 的取捨）。
- **`realtime-metrics` 只保留 10 分鐘。** 這些是瞬時值，權威數字在 `order_db`、
  聚合在讀取模型裡。驗收腳本因此需要一個時間錨點。
