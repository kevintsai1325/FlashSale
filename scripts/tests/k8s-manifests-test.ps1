...
Assert-True ($resources.Count -eq 33) "Expected exactly 33 rendered resources, got $($resources.Count)."
...
$expectedWorkloads = @('postgres', 'postgres-purchase', 'redis', 'rabbitmq', 'kafka', 'mailpit', 'zipkin', 'backend', 'purchase-service', 'frontend', 'nginx')
$workloads = @($resources | Where-Object { $_.kind -in @('Deployment', 'StatefulSet') })
Assert-True ($workloads.Count -eq 11) "Expected exactly eleven workloads, got $($workloads.Count)."
...
$singleReplicaWorkloads = @('postgres', 'postgres-purchase', 'redis', 'rabbitmq', 'kafka', 'mailpit', 'zipkin', 'frontend', 'nginx')
...
foreach ($name in @('postgres', 'postgres-purchase', 'redis', 'rabbitmq', 'kafka', 'zipkin', 'backend', 'purchase-service')) {
...
Assert-True ($statefulSets.Count -eq 5) 'Expected exactly five StatefulSets.'
...
Write-Host 'PASS: Kubernetes rendered-resource contract (33 resources, exact stages, minimal RBAC, 11 workloads, probes, persistence, headless Services, Secret refs, namespace, local images, backend rollout strategy, backend PodDisruptionBudget, liveness timeout, the HPA/connection-pool ceiling, and the purchase-service split).'
