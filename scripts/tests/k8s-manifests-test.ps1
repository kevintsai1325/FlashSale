...
$expectedStages = @{
    ...
    dependency = @(
        ...
        'Service/postgres-order-headless', 'Service/postgres-order', 'StatefulSet/postgres-order',
...
foreach ($name in @('postgres', 'postgres-purchase', 'postgres-analytics', 'postgres-order', 'redis', 'rabbitmq', 'kafka', 'zipkin', 'backend', 'purchase-service', 'order-service', 'analytics-service')) {
...
Assert-True ($statefulSets.Count -eq 7) 'Expected exactly seven StatefulSets.'
