...
class OrderPurchaseConsumerTest {
    ...
    void setUp() {
        consumer = new OrderPurchaseConsumer(consumedMessageGuard, inventoryRepository, orderRepository,
            orderStatusHistoryRepository, objectMapper, orderMetrics, outboxWriter,
            orderEventPublisher);
    }
    ...
    /**
     * P5：商品名稱從事件裡拿，不再查 catalog —— 那張表已經是 platform 的資料。
     * 事件裡的名稱是「下單當下」的名稱，本來就比事後查一次更正確。
     */
    @Test
    void savesTheProductNameFromTheEventAsTheOrderItemSnapshot() throws Exception {
        AtomicReference<Order> savedOrder = new AtomicReference<>();
        stubHappyPath(savedOrder);

        consumer.handle(messageFor(new CreateOrderRequestedEvent(REQUEST_ID, 202L, 303L, 404L,
            "限量鍵盤", 1, new BigDecimal("1999.00")), EVENT_ID));
