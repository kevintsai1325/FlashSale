...
class AuthControllerLoginIT {
    ...
    void loginWithValidCredentialsReturnsAccessTokenAndRefreshCookie() throws Exception {
        mockMvc.perform(post("/api/auth/register").contentType(APPLICATION_JSON)
                .content(requestBody("erin@example.com", "secret123")))
    ...
    @Test
    void loginWithWrongPasswordReturns401WithInvalidCredentialsCode() throws Exception {
        mockMvc.perform(post("/api/auth/register").contentType(APPLICATION_JSON)
                .content(requestBody("frank@example.com", "secret123")))
            .andExpect(status().isCreated());

        mockMvc.perform(post("/api/auth/login").contentType(APPLICATION_JSON)
                .content(requestBody("frank@example.com", "wrongpassword")))
